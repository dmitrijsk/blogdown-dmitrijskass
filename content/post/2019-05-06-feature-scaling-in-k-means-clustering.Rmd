...
For example, we are given two points with coordinates (1, 2) and (3, 4). Then the Euclidean distance $d$ between these points is approximately 2.8:
...
Let's take k-means clustering. The algorithm aims to partition observations into *k* groups such that the sum of squared *distances* from observations to the assigned cluster centres is minimized. In the example plotted below, there are two distinct clusters ($k=2$) with 10 observations in each and with cluster centers highlighted with coloured crosses.

```{r plot_clusters, echo = FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=4, fig.align='center'}
set.seed(1)
x <- tibble(x = c(rnorm(n = 10, mean = 0.5, sd = 0.2), rnorm(n = 10, mean = 2.5, sd = 0.2)),
       y = c(rnorm(n = 10, mean = 0.5, sd = 0.2), rnorm(n = 10, mean = 2.5, sd = 0.2)))
...
x %>% 
  ...
  labs(subtitle = "Hypothetical clusters, k = 2",
...
Below is the result from k-means. iPhone 8 Plus is alone in cluster 1 and three other phones are in cluster 2:
...
phones_orig_clusters <- phones_df %>% 
  mutate(cluster = kmeans_fit$cluster)

p <- phones_orig_clusters %>% 
...
To summarize, here are side-by-side k-means clustering results for $k=2$ with original variables (plot A on the left) and standardized variables (plot B on the right):
