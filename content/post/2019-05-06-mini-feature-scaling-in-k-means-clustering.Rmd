...
There are many possible ways to define the concept of "(dis)similarity", but by far the most common choice involves [Euclidean distance](https://en.wikipedia.org/wiki/Euclidean_distance). If the points $(x1, y1)$ and $(x2,y2)$ are in 2-dimensional space, then the Euclidean distance between them is 
...
For example, if the distance between point A (1,1) and point B (3,4) is $\sqrt{(3-1)^2+(4-2)^2}=\sqrt{8}:

```{r distance_example, echo = FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=4, fig.align='center'}
library(tidyverse)
...
example %>% 
  ...
  geom_point(size = 3) +
  coord_equal() +
  scale_x_continuous(breaks = 1:5, expand = expand_scale(add = 2), minor_breaks = NULL) +
  scale_y_continuous(breaks = 1:3, expand = expand_scale(add = 1.5), minor_breaks = NULL) +
  # Arrows
  annotate(geom = "segment", x = 1, y = 1, xend = 2, yend = 3, arrow = arrow(length = unit(0.3, "cm"))) +
  annotate(geom = "segment", x = 1, y = 1, xend = 5, yend = 1, arrow = arrow(length = unit(0.3, "cm"))) +
  # Coordinates.
  annotate(geom = "text", size = 3.5, x = 0.6, y = 1, label = "A (1,1)", hjust = 1) +
  annotate(geom = "text", size = 3.5, x = 4.3, y = 1, label = "B (5,1)", hjust = -1) +
  annotate(geom = "text", size = 3.5, x = 1.1, y = 3, label = "C (3,2)") +
  # Lengths.
  annotate(geom = "text", size = 3.5, x = 4.0, y = 2.0, label = expression(sqrt((3-1)^2 + (2-1)^2) == sqrt(5) %~~% 2.24)) +
  annotate(geom = "text", size = 3.5, x = 3.1, y = 0.4, label = expression(sqrt((5-1)^2 + (1-1)^2) == 2)) +
