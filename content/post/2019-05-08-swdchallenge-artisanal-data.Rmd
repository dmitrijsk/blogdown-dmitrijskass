...
This post is a participation in [#SWDchallenge: artisanal data](http://www.storytellingwithdata.com/blog/2019/5/1/swdchallenge-artisanal-data). Along with that it visualizes the **tf-idf - effect of multiplying tf by idf**. Variations of tf-idf, term frequency-inverse document frequency, are often used by search engines and text-based recommender systems.
...
If you are not familiar with tf (term frequency) and idf (inverse document frequency) then they are described [below](#about-tf-idf) with examples. Otherwise, here is the illustration:
...
gg_subtitle <- paste(str_wrap(
  "These are top an bottom six most important words in Andy Cotgreave's interview in the 'learning dataviz' episode of #SWDpodast. Although words such as 'data' and 'learn' are the 1st and the 5th most frequent, they are not important when viewed in the context of other 11 interviews on the same podcast episode. This is because all 12 interviews are about learning data visualization.", width = 77),
  "",
...
For example, if the word "learn" is used 8 times and the interview contains 540 words then tf equals

$$tf(learn)=8/540\approx0.015.$$

The shortcoming of tf is that it is calculated using a single document in isolation from other documents in the collection. 
...
It means that the word "learn" is not important in the context of 12 interviews. Other words that have zero importance here are *`r setdiff(zero_tfidf, "learn")`*.

**Term frequency-inverse document frequency (tf-idf)** is a multiplication of tf and idf:

$$tfidf(t) = tf(t) \times idf(t).$$

It does not matter how high $tf(learn)$ is, if $idf(learn)=0$, its $tfidf(learn)$ will also be zero.


Notice that the value of a term's tf is **local** (document-specific), while its idf is **global** (single value within a collection of documents). For example, tf of the word "learn" in Andy's interview is 0.015 and 0.001 in Jeffrey's interview. Any word's idf is constant within the collection of documents - for the "word" it is zero. Therefore, no matter how often it appears in any single interview, its tf-idf is always zero.

Here is the data used for [the plot above](#tfidf) - top an bottom six most important words in Andy Cotgreave's interview:
...
t <- tokens_tfidf %>% 
  ...
  mutate(word_tf_rank = row_number(-tf),
         word_tfidf_rank = row_number(-tf_idf)) %>%
  filter(word_tf_rank %in% 1:6 | word_tfidf_rank %in% 1:6) %>% 
  mutate_if(is.double, function(x) round(x, 3)) %>% 
  ungroup() %>% 
  select(word_lemma, freq = n, total_words, tf, idf, tf_idf, tf_rank = word_tf_rank, tfidf_rank = word_tfidf_rank) %>% 
  arrange(desc(freq))

cols <- c("word", "count", "total\nwords", "tf", "idf", "tf-idf", "rank\nby tf", "rank\nby idf")
knitr::kable(t, format = "html", col.names = cols) %>% 
