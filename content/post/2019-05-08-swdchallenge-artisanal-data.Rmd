...
If you are already familiar with tf (term frequency), idf (inverse document frequency) and tf-idf (term frequency-inverse document frequency) then you may skip this section and go directly to the [visual comparison of two measures of word importance](#tfidf).
...
$$tf(learn)=8/540\approx0.015.$$

When used to measure word importance, the shortcoming of tf is that it is calculated using a single document in isolation from other documents in the collection. Notice that a term's tf may be different for each document within a collection.
...
$$idf(learn)=log_e(12/12)=log_e(1)=0$$
...
Notice that idf for each term is constant within a collection of documents.
...
It does not matter how high $tf(learn)$ is, if $idf(learn)=0$, the $tfidf(learn)$ will also be zero. On the contrary, if a word is mentioned only in one interview from 12 then its tf get multiplied by `r round(log(12/1), 2)`. This is what happened to the word "copy". Its importance rank by tf and tf-idf is #40 and #3, respectively.
...
# tf-idf: the effect of idf on tf {#tfidf}

The plot below depicts changes in ranks for the most important words in Andy Cotgreave's interview in the "learning dataviz" episode of #SWDpodast measured by tf (on the left) and tf-idf (on the right). Tf gives the highest weight to words that are simply used most frequently. It is a local measure in a sense that it ignores other interviews in the same podcast episode. 

Multiplication of tf by idf, which gives tf-idf, diminished the importance of words that are common across all interviews, such as "data" and "learn". At the same time is amplifies the importance of rare words, such as "copy" and "fun". 

A word "copy" is used only in Andy's interview and he uses it three times. It is used in the context of recommending Austin Kleon's book "Steal Like an Artist". A word "fun" is used in two other interviews, but Andy uses is most frequency. He recommends listeners to *"get out of your comfort zone and have fun"*. And continues with saying *"Have fun. My gosh, you are allowed to have fun!"*. Let's do it!


```{r message=FALSE, warning=FALSE, echo = FALSE, fig.width=5, fig.height=6, fig.align='center'}
library(ggrepel)
...
slopes_tf_tfidf <- tokens_tfidf %>% 
  ...
  mutate(word_tf_rank = rank(-tf, ties.method = "min"),
         word_tfidf_rank = rank(-tf_idf, ties.method = "max")) %>%
  filter(row_number(-tf) %in% 1:6 | row_number(-tf_idf) %in% 1:6) %>% 
  ungroup()
...
Below is the data used for plotting. There are 12 interviews. Andy Cotgreave's interview contains 540 words, 271 of them unique.
...
cols <- c("word", "count", "total words", "tf", "documents", "idf", "tf-idf", "rank by tf", "rank by idf")
