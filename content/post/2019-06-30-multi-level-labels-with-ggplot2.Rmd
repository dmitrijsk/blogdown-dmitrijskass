...
```{r demo, echo = FALSE, fig.height = 5, fig.width = 6, fig.align='center'}
library(dplyr)
library(ggplot2)
library(stringr)
...
Below I discuss alternative solution to multi-level labels for these two charts. I deliberately keep them simple with no customization of colours, etc.
...
p_line + 
  scale_x_date(date_breaks = "1 month",  # Date labels for each month.
               date_labels = "%b %y")    # Date format: short month and a 2-digit year.
```
...
1. Gets the first letter of each month's name. Depending on your locale you may or may not need to capitalize this letter. I would need it if I kept "latvian" locale, so I keep it inside my function below.
2. Gets the year part from each date.
3. Pastes the years behind months if 
    ...
    * the previous date in a vector has a different year component (does not have to be January).
    ...
    The separator is a new line symbol `\n`, which wraps the text and makes it <i>look</i> like a multi-line label. However, it is still a single vector of x axis labels, whose appearance may be manipulated with `theme(axis.text.x = element_text(...))`. 
