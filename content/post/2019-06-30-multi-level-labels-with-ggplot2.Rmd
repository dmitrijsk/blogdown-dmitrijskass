...
Recently I faced a necessity to create **multi-level labels with ggplot2** and had no idea how to do it. Multi-level labels imply some sort of hierarchical structure in data. For example, survey questions may be assigned into groups; and dates on the timeline may be assigned into years. A 15-minute Google-fu provided me with various solutions described on Stack Overflow that worked with varying success for different types of charts. An important aspect is whether data points between groups should be connected. The bar chart (plot A below) is an example where data points from different groups should not be connected and a line chart (plot B below) is an example where data points should be connected both within and between groups:
...
Below I discuss possible solution to multi-level labels for these two charts. I deliberately keep them simple with no customization of colours, etc.

# Bar chart

First, let's simulate the data for the bar chart. Let it be a result of a hypothetical survey with `r sum(TIMES)` questions labelled from Q1 to `r paste0("Q", sum(TIMES))` (`question` column) and combined into `r length(TIMES)` groups (`group` column). These will substitute x axis labels. The y axis values are drawn from a uniform distribution and represent a proportion of respondents that correctly answered corresponding questions (`proportion` column).
...
Why would you want to add a grouping variable? To make the chart more intuitive for the reader by clearly showing that questions belong to different groups.
...
Done. We may remove the white space between facets by providing a `panel.spacing = unit(0, units = "cm")` argument to the `theme()` function. But I don't recommend it because the borders between groups get blurred and readability of the chart suffers.
...
Again, we start with simulating data for the line chart. Let it be sales volume of some healthy product during 16 months from November 2018 to February 2020. I deliberately chose this period as it partly covers three years.
...
Your x axis labels may vary depending on regional settings. My default region is Latvia. Locale can be changed with `Sys.setlocale()`:
...
Why would you want to use multi-level labels for x axis in this case? Because labelling each month creates a lot of clutter even with a short date format:
...
* remove the white space between the facets with `theme(panel.spacing = unit(0, units = "cm")`,
* and remove the expansion of x axis with `scale_x_date(expand = expand_scale(0, 0))`,

the end data points of facets will still be disconnected. Although this discontinuity may not be apparent if vertical changes between the end data points of facets are small enough.
...
[here](https://stackoverflow.com/questions/44616530/axis-labels-on-two-lines-with-nested-x-variables-year-below-months/44616739#44616739) appears overly complicated to me. Instead, I offer a different solution, which seems to me simple and generalizable, i.e. does not depend on a particular data set.


## Example with text wrap

A simple approach is to modify x axis labels with a date formatting function. Suppose we have a date vector. The function does three things:
...
3. Pastes the years behind months if 
    ...
    * the previous date in a vector has a different year component (does not have to be January).
...
# Test the function on quarterly data.
...
Let's start from the beginning with a simple line chart. Then supply the above defined date formatting function to `scale_x_date(labels = ...)` among few other arguments. As the final touch, left-justify the labels with the `theme()`.
