...
Even if we remove the white space between the facets with `theme(panel.spacing = unit(0, units = "cm")`, the end data points of facets will still be disconnected. Notice the discontinuities between Decembers and Januaries in the chart below:

```{r line_facets_no_whitespace}
# Still does not suit.
data %>% 
  ggplot(aes(x = date, y = sales)) +
  geom_line() +
...
A simple approach is to modify x axis labels with a text (in this case - dates) formatting function. Suppose we have a date vector. The function does three things:
