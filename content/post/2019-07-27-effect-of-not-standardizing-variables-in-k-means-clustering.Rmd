...
Different standardization methods are available, including z-standardization (also called z-score standardization) and range standardizaton. Z-standardization rescales each variable by subtracting the mean $\bar{x}$ and dividing by its standard deviation $s$:
...
In cluster analysis, however, range standardization (e.g., to a range of 0 to 1) typically works better [@milligan_cooper]. Range standardization requires subtracting the minimum value of every observation and then dividing it by the range (i.e., the difference between the maximum and minimum value):
...
Notice each column now has a range of one.

Now repeat k-means clustering with $k=2$ using standardized variables.
...
phones_df %>%
  mutate(approx_price_eur_scaled = standardize_range(approx_price_eur),
...
iPhone 4 and 6 are now in cluster 1 and iPhone 8Plus and 6s Plus are in cluster 2. Below are distances calculated from standardized units:
...
Beware of differences in variation of variables used for clustering as variables with higher variation have a higher impact on clustering. The choise of variable standardization methods should depend on the type of data and problem at hand and there is no single best method of standardization. As [@james_witten_hastie_tibshirani_2017, pp. 385] recommend, always try different standardization methods as see which offers most valuable insights. [@milligan_cooper] suggest that range standardization typically works better for clustering.
