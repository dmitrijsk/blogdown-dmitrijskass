...
Now this clustering may look correct. However, the question is, **do you want the scales of measurements influence the clustering results?** Consider the same data set with prices being converted to Indonesian rupiah (IDR) at 16,000 rupiah for 1 euro. Then this clustering problem would essentially become 1-dimensional as even 1 euro difference in price stated in rupiah would outweight any possible difference in weight. In other words, price would be the only variable essentially affecting the k-means clustering. Even with prices being stated in euros, price affects clustering more than the weight just because if its higher variability.

<!-- Consider another example - clustering of persons according to weight and height. If height is measured in meters and weight in grams then this would produce very different clustering results compared with the same pers -->



# Standardized variables

There are different ways for standardizing the variables. One of th most popular ones is standardizing by subtracting the mean $\bar{x}$ (i.e. centering) and dividing by its standard deviation $s$:

$$Z=\frac{X-\bar{x}}{s}.$$

After this transformation the standard deviation if each variable $s$ equals one, which makes price and weight equally important for k-means clustering. This type of standardization is done with `scale()`:
...
Notice that althouh the scales changed, the rankings within each variable stayed unchaned.

We now repeat k-means clustering with $k=2$ using standardized variables.
...
iPhone 8Plus and 6s Plus are now in cluster 1 and iPhone 4 and 6 are in cluster 2. Below are distances calculated from standardized units. 
...
Conclusions. Does it mean that you should always standardize your variables? Not necessarily. You may want some of your variables to have a higher impact on clustering than other variables.

But you should always consider some type of standartization.
