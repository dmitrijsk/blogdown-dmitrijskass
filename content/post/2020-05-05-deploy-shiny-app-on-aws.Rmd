...
```{r}
public_ip <- "54.172.160.16"
```

Shiny applications are used extensively, including [education](http://www2.stat.duke.edu/~mc301/shinyed/) and business. For example, Data Science team in Creamfinance uses Shiny apps to streamline the modelling process from data collection to model selection. It allows removing the R programming part from processes that are well-established, which adds a lot to standardization and efficiency.

So you have built a Shiny app and it runs nicely on your personal computer. How do you make it available to your audience? This tutorial shows how to deploy Shiny apps on a virtual server in the cloud using Amazon Web Services (AWS), which is one of many providers of such service.

The outline is the following:

1. Launch a virtual server using AWS.
2. Connect to a virtual server.
3. Install R and packages.
4. Install Shiny Server
5. Clone your git repo.
6. Deploy your own Shiny apps.
7. FAQ.

Before we start, why AWS? AWS is one of leaders in the marketplace of cloud computing. It has a wide range of products, including A, B and C. So if you plan using other cloud services in the future then learning a bit of AWS now will benefit your other projects as well.

Elastic Compute Cloud (EC2) is a secure and resizable virtual server in the cloud, which is what we need here. EC2 is free to try for one year. And there are lots of educational resources, including [Coursera](https://www.coursera.org/specializations/aws-fundamentals) and [edX](https://www.edx.org/school/aws) MOOCs by AWS.
...
2. AWS offers a broad set of [cloud-based products](https://aws.amazon.com/products). We are interested in EC2 (Elastic Compute Cloud), which is a virtual server in the cloud, also called an instance. Click on **EC2** in the Compute product category to open the EC2 Dashboard.
...
[^6]: [AWS documentation. "Public IPv4 Addresses and External DNS Hostnames"](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-instance-addressing.html#concepts-public-addresses). 

2. My IPv4 Public is `r public_ip`. If you are a Windows user, as I am, you will need an SSH client to connect to your instance. We will use PuTTY, which is available for free at https://www.putty.org/.
...
4. Open PuTTY, enter the IPv4 Public of your instance into **Host Name (or IP address)**, `r public_ip` in my case. The default **Port** is 22 and your EC2 instance must be ready to establish a connection using the same port number. 
...
Go to Connection > SSH > Auth, press **Browse** and select your private key. Go back to Session, type the name of the instance into **Saved Sessions** and press **Save**. This configuration will be saved and available for you next time after pressing **Load**. Alright, press **Open**.

```{r fig.align='center', fig.cap = "PuTTY Configuration - Auth."}
knitr::include_graphics("/img/aws/putty_auth.png")
```

The terminal will open. All further communication between you and your instance will happen in this terminal.
...
Note: `su` stands for "switch user". Invoked without a username, `su` defaults to becoming the superuser. In effect, these two commands[^3] install the packages from under the root.
...
3. To open the port go back to the **EC2 Dashboard** in your browser. Click on **Running instances**, select your instance, find the **Security Groups** column and click on **launch-wizard-1**. Find the **Inbound rules** and add one more rule like in a figure below.
...
4. Click on the "minimum-variance-portfolio-cor" to open the app demonstrating the influence of correlation between two investment asset returns on the mean and variance of portfolio returns.

Congratulations! Let's summarize what you have successfully done:

1. Created an account with AWS and launched a virtual server in the cloud (instance).
