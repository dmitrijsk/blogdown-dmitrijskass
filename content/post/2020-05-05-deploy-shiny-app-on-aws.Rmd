...
3. PuTTY doesn't natively support the private key format (.pem) generated by Amazon EC2. You must convert your private key into a .ppk file before you can connect to your instance using PuTTY. You can use the PuTTYgen tool (PuTTY Key Generator) for this conversion[^7].
    ...
    Press **Load**, optionally create a key passphrase (i.e. password-protect your private key), press **Generate** and then **Save private key**. In case of an issue read detailed instructions [here](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/putty.html).
...
1. Install RStudio Server to write code directly on the remote machine because.
