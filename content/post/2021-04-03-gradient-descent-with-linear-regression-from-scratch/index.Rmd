...
With learning rate $\alpha=0.001$ the algorithm slowly converges in 772 iterations. The surface plot below is the surface of the cost function $J(\mathbf{w})$. Red dots are updated parameters from initial $\mathbf{w}^{(0)} = [0,0]$ to optimal $\mathbf{w}^{(772)} = [0.24, 0.84]$.
...
In the [next post](/2021/04/15/adagrad-adaptive-gradient-algorithm/) we will look at the adaptive gradient algorithm AdaGrad, which performs more informative gradient-based learning by adapting the learning rate for *each predictor individually* and does not require guessing $\alpha$.
