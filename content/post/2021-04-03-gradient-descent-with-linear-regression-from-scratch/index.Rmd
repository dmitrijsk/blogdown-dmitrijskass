...
<p style="font-family:'Merriweather';color:blue;font-size:70%;">
Add: Loss function as a separate expression. (2021-04-22).
</p>
...
where $n$ is the number of samples in the training data. It is is a scalar-valued function. Quadratic nature of the loss function produces a [convex](https://en.wikipedia.org/wiki/Convex_function) cost function.
...
\end{array}\right]=
\left[\begin{array}{c}
2 \sum_{i=1}^{n}(y_i - (w_0 + w_1 x_{1i} + w_2 x_{2i})) (-1)\\
\\
2 \sum_{i=1}^{n}(y_i - (w_0 + w_1 x_{1i} + w_2 x_{2i})) (-x_{1i})\\
\\
2 \sum_{i=1}^{n}(y_i - (w_0 + w_1 x_{1i} + w_2 x_{2i})) (-x_{2i})
\end{array}\right]$$
...
<p style="font-family:'Merriweather';color:blue;font-size:70%;">
Fix: Comment for $eps$ is outdated. (2021-04-22)
</p>
...
Line 55 initializes parameters $\mathbf{w^{(0)}}$ as a zero vector. Initialization can be done arbitrarily here because we are dealing with a convex loss function. For convex problems there is only one stationary point, which also is the global minimum.
...
<center>
![](images/learning_rate_0.001.gif){width=80%}
</center>
...
<center>
![](images/learning_rate_0.005.gif){width=80%}
</center>
...
<center>
![](images/learning_rate_0.01.gif){width=80%}
</center>
...
<center>
![](images/learning_rate_0.02.gif){width=80%}
</center>
