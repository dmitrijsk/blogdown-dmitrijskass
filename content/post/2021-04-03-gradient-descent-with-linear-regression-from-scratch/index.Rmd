...
Returns the gradient vector of shape $[p+1, 1]$. `d_intercept` is a partial derivative of the cost function w.r.t. the intercept $w_0$. `d_x` is a partial derivative of the cost function w.r.t. to input variables ($w_1, \dots, w_p$).
...
Line 61-72 implement the gradient descent algorithm. Line 63 calculates the gradient, line 64 updates the parameters, line 66 calculates the value of the cost function. An early stoppage occurs when the Euclidean norm of the difference between the gradient descent solution and least squares solution passes the tolerance level. Alternatively, we could use the last two values of the cost function - stop if a decrease is less than some tolerance level.
...
In the next post we will look at the adaptive gradient algorithm AdaGrad, which performs more informative gradient-based learning and converges much faster.
...
[^sml]: Lindholm, A., Wahlström, N., Lindsten, F., & Schön, T. B. (2020). Supervised machine learning. Chapter 5 "Learning parametric models". *Unpublished*. [Link](http://smlbook.org/)
