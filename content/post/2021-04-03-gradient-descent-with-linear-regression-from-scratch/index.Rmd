...
* In the end, Adagrad is used to walk away for the dark art of guessing learning rates.
...
Implements the gradient descent algorithm. Line 6 calculates the gradient and line 7 updates the parameters.

The value of parameters and cost at each iteration are saved in lines 8 and 11 for visualization purposes. 

The number of iterations is limited by `self.max_iterations`. An early stoppage in lines 14-15 occurs when the Euclidean norm of the difference between the gradient descent solution and least squares solution passes the tolerance level. Alternatively, we could use the last two values of the cost function - stop if a decrease is less than some tolerance level.
...
**Note:** This surface plot also reveals that the gradient descent algorithm does not really slide down the surface. In fact, it is a step-wise movement:
...
* Then the cost is recalculated using $[ w_0^{(1)}, w_1^{(1)}]$ and the red dot descends to $[ w_0^{(1)}, w_1^{(1)},J(w_0^{(1)}, w_1^{(1)}) ]$, which is $[0.05, 1.4, 6.3]$. But it is not guaranteed to descent. If the step size in $XY$ plane was too large, the red dot would actually ascend and the algorithm would not converge. 
...
We would like to eliminate this dark art and stop guessing the right value for $\alpha$. A safe (but sometimes slow) choice is to set $\alpha = \frac{i_0}{i}$, where $i_0$ is any initial positive value and $i$ is the iteration counter. This guarantees that it will eventually become small enough to converge.[^6] With the same generated data and $i_0=1$ the algorithm converged in 278,951 iterations (slow indeed).


# Adagrad - No More Guessing

When predictors have different scales (e.g., milligrams and tons), the gradient will mostly point in the direction of the predictor on the larger scale. As a result, the algorithm may take a long time to converge. [Adagrad](https://jmlr.org/papers/v12/duchi11a.html) (Duchi et al., 2011) sets the step size adaptively for each predictor separately. It keeps a running average of the squared gradient magnitude and sets a small step size for predictors that have large gradients, and a large step size for predictors with small gradients[^7].

## Implementation

**`__init__` method**

Add the $G$ variable to keep a running average of the squared gradient magnitude.

<script src="https://gist.github.com/dmitrijsk/b355159ede76bc8086e24e18bb96d2ca.js"></script>

**`adagrad` method**

Implements the adjustment for the gradient.

<script src="https://gist.github.com/dmitrijsk/d7aef32138a01d6145ac456ca8f309de.js"></script>

**`fit` method**

Add line 7 to the existing `fit` method. The end result looks as follows:

<script src="https://gist.github.com/dmitrijsk/2a54c374a94912c38ccce304774d5d58.js"></script>
