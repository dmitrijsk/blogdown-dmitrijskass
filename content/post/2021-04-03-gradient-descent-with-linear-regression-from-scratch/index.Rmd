...
where $\nabla J(\mathbf{w})$ is the gradient of $J$. This approximation is valid only when the step size $\mathbf{s}$ is small[^gradient-descent-and-beyond]. We will return to this in the learning rate discussion.
...
Consequently, $-\nabla J(\mathbf{w})$ points in the direction of the steepest descent. Setting $\mathbf{s} = -\alpha \nabla J(\mathbf{w})$ for a *sufficiently small* $\alpha>0$ guarantees to decrease the function[^see-one]:
...
where $n$ is the number of samples in the training data. It is a scalar-valued function. If $p=2$  then 
...
Returns the gradient vector. `d_intercept` is a partial derivative of the cost function w.r.t. the intercept $w_0$. `d_x` is a partial derivative of the cost function w.r.t. to input variables ($w_1, \dots, w_p$).

### `fit`

Line 55 initializes initial parameters $\mathbf{w^{(0)}}$ as a zero vector. Initialization can be done arbitrarily here because we are dealing with a convex loss function. For convex problems there is only one stationary point, which also is the global minimum.
...
"Setting the learning rate $\alpha>0$ is a dark art. Only if it is sufficiently small will gradient descent converge"[^see-one]. Below we take a look at how a choice of $\alpha$ impacts convergence.

Let's generate $\mathbf{X}$ and $\mathbf{y}$ data for the linear regression and use gradient descent to fit a straight line. Full code is available at my [GitHub repository](https://github.com/dmitrijsk/blogdown-dmitrijskass/tree/gradient-descent-with-linear-regression-from-scratch/content/post/2021-04-03-gradient-descent-with-linear-regression-from-scratch/code). Generated data looks as follows:

<center>
![](images/1-generated-data.png){width=70%}
</center>

We fit the line using the gradient descent with `GradientDescentLinearRegression().fit(X, y)`.


## Slow convergence

With learning rate $\alpha=0.001$ the algorithm slowly converges in 772 iterations. The surface plot below is the surface of the cost function $J(\mathbf{w})$. Red dots are updated parameters from initial $\mathbf{w}^{(0)} = [0,0]$ to optimal $\mathbf{w}^{(1001)} = [0.24, 0.84]$.
...
Learning rate $\alpha=0.005$ seems to be a better guess and the algorithm converges in 184 iterations. After the first three steps the parameters are fairly close to the optimal solution.
...
* After the first update of the parameters, imagine the red dot moving in the $XY$ plane to $[ w_0^{(1)}, w_1^{(1)},J(w_0^{(0)}, w_1^{(0)}) ]$, which is $[0.05, 1.4, 13.5]$.
* Then the cost is recalculated using $[ w_0^{(1)}, w_1^{(1)}]$ and the red dot descends to $[ w_0^{(1)}, w_1^{(1)},J(w_0^{(1)}, w_1^{(1)}) ]$, which is $[0.05, 1.4, 6.3]$. But it is not guaranteed to descent. If the step size in $XY$ plane was too large, the red dot would actually ascend and the algorithm would diverge. 
...
The fitted line does not seem to move during the first 20 iterations due to a huge scale of the plot (1e+10). However, later we see that the line starts to swing wildly and quickly approaches a vertical where the cost becomes infinitely large.
...
We would like to get rid of the dark art of guessing learning rates. A safe (but sometimes slow) choice is to set $\alpha = \frac{i_0}{i}$, where $i_0$ is any initial positive value and $i$ is the iteration counter. This guarantees that it will eventually become small enough to converge[^see-one]. With the same generated data and $i_0=1$ the algorithm converged in 278,951 iterations (slow indeed).

In the [next post](/2021/04/15/adagrad-adaptive-gradient-algorithm/) we will look at the adaptive gradient algorithm AdaGrad, which performs more informative gradient-based learning by adapting the learning for each predictor individually and does not require guessing $\alpha$.
...
[^see-one]: See 1.
[^gradient-descent-and-beyond]: "Gradient Descent (and Beyond)". Lecture notes for the "Machine Learning for Intelligent Systems" course at Cornell University. [Link](https://www.cs.cornell.edu/courses/cs4780/2018fa/lectures/lecturenote07.html)
[^partials]: Havens, A. Multivariate functions and partial derivatives. [Link](https://people.math.umass.edu/~havens/Partials.pdf)
[^sml]: Lindholm, A., Wahlström, N., Lindsten, F., & Schön, T. B. (2020). Machine Learning - A First Course for Engineers and Scientists. Chapter 5 "Learning parametric models". *Unpublished*. [Link](http://smlbook.org/)
