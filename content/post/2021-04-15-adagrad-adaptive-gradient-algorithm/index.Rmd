...
One of the problems of the standard gradient descent is a difficulty of choosing a suitable learning rate. Another problem is that it treats all features as equal. But they are not![^stanford-lecture]. AdaGrad[^adagrad] is one of the gradient-based optimization algorithms that aim to solve both of these problems.
...
$$\mathbf{G}^{(i)} = \mathbf{G}^{(i-1)} + \mathbf{g}^{(i)} \cdot \mathbf{g}^{(i)},$$

where $\mathbf{G}^{(i)}$, sometimes called cache, is a vector of the same length as $\mathbf{g}^{(i)}$ and gradient is squared element-wise. Then the update rule is 
...
|   **Input:** Cost function $J(\mathbf{w})$, initial $\mathbf{w}^{(0)}$, initial $\mathbf{G}^{(0)}$, constants $\eta$ and $\epsilon$.
|   **Result:** $\widehat{\mathbf{w}}$.
| $\texttt{1}$ Set $i \leftarrow 0$
| $\texttt{2}$ **while** $\lVert \mathbf{w}^{(i)} - \mathbf{w}^{(i-1)} \rVert > \epsilon$  **do**
| $\texttt{3}$ $\text{    }$ Compute gradient vector $\mathbf{g}^{(i)} = \nabla J(\mathbf{w}^{(i)})$
| $\texttt{4}$ $\text{    }$ Update cache $\mathbf{G}^{(i+1)} \leftarrow \mathbf{G}^{(i)} + \mathbf{g}^{(i)} \cdot \mathbf{g}^{(i)}$
| $\texttt{5}$ $\text{    }$ Update parameters $\mathbf{w}^{(i+1)} \leftarrow \mathbf{w}^{(i)}-\frac{\eta}{\sqrt{\mathbf{G}^{(i+1)} + \epsilon}} \mathbf{g}^{(i)}$
| $\texttt{6}$ $\text{    }$ Update $i \leftarrow i + 1$
| $\texttt{7}$ **end**
| $\texttt{8}$ **return** $\widehat{\mathbf{w}} \leftarrow \mathbf{w}^{(i-1)}$



# Implementation in Python

Only a few changes need to be implemented in the gradient descent code with linear regression from [the previous post](/2021/04/03/gradient-descent-with-linear-regression-from-scratch/). Cache is initialized as a zero vector in line 56 and updated in line 63. The step size is calculated in lines 64-65. $\eta$ is usually set at 0.01[^ruder] and $\epsilon$ is a small number, e.g., 1e-6.
...
With $\eta=0.01$ and $\epsilon=1e-6$ the algorithm converges in 10,578 iterations. However, the beauty of AdaGrad is that it eliminates the need to manually tune the learning rate. With $\eta=1$ it converges in 11 iterations (illustrated below) and with an extremely high $\eta=10$ is still converges in 12 iterations.
...
[^gd-and-beyond]: [Gradient Descent (and Beyond)](https://www.cs.cornell.edu/courses/cs4780/2018fa/lectures/lecturenote07.html)
...
[^stanford-lecture]: https://web.stanford.edu/~jduchi/projects/DuchiHaSi12_ismp.pdf
