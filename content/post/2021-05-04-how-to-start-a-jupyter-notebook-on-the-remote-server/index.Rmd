...
<!-- !! Replace public it with "remote-ip" !! -->
<!-- !! Ne vazhno private ili public, prosto ip udaljennogo kompa, k kotoromu ja mogu podsoedenitjsa. !! -->
<!-- !! Naprimer eto mozhet bitj komp na sosednem stole, k kotoromu ja mogu prisoedenitjsa cherez private IP !! -->

<!-- !! https connection, bez tunelja !! eto podkljuchenije mezhdu prilozhenijami -->

We want to launch computationally expensive operations on a more powerful remote machine or a remote machine connected to a cluster. It can be a virtual machine on the cloud (e.g., AWS EC2) or a physical machine on a neighboring desk. The main thing is that you can access it using a remote IP. And we want to use a Jupyter notebook in a local browser to make coding and experimenting more pleasant. There are at least two ways to access Jupyter server that runs on a remote machine through a browser on a local machine:

* using SSH port forwarding and `http://localhost:8888` in your browser, and
* opening the 8888 port on a remote machine and using `https://<remote-ip>:8888`.
...
If you don't want to make your notebook accessible through the internet, you may use SSH local port forwarding to map the port 8888 on your local machine to the same port on the remote machine. This is accessibly illustrated in a sketch below taken from [the answer at unix.stackexchange.com](https://unix.stackexchange.com/questions/115897/whats-ssh-port-forwarding-and-whats-the-difference-between-ssh-local-and-remot/115906#115906):
...
* `-N` option prevents the execution of a remote command.
...
A `<user>` is the one used to launch Jupyter on the remote machine.
...
Done. Open the browser on your local machine and go to `https://<remote-ip>:8888`. Note that when you enable SSL support, you will need to access the notebook server over `https://`, rather than plain `http://`.

I tried to make a sketch in the manner of a sketch above:
