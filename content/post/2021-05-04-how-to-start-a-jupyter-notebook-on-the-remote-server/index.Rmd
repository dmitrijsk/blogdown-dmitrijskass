...
There are two methods to connect to a Jupyter notebook running on a remote Ubuntu machine through the browser on your local machine: port forwarding and public IP. You may skip the installation part if `jupyter notebook` already works.
...
1. Open the file for editing with `sudo nano ~/.bash_profile`. This will also create a file if it does not exist.
...
3. Access it through the browser on your local machine using the public IP of the remote machine `http://<public-ip>:8888`.
...
1. Create a config file in `~/.jupyter/`:
...
2. Protect your notebook with a password. Prepare a hashed password in Python:
    ...
    >>>> password
    'sha1:3b6a76a30e8c:c7b277c67471730132f83cb45a95ab462495a2f6'
   ```
   
   Open the config file for editing, find a line with `c.NotebookApp.password`, uncomment and replace '' with your hashed password:
    ...
    #  Default: ''
    c.NotebookApp.password = u'sha1:3b6a76a30e8c:c7b277c67471730132f83cb45a95ab462495a2f6'
   ```
...
### Listing and killing Jupyter from the terminal
