...
Generating data allows to set the true values of (usually) unknown variables. And knowing the true values of these variables can be useful in experiments, for example, to check if a model is able to learn them. In this post we generate data for the binary logistic regression and discuss the source of randomness in it.
